...
def onek_encoding_unk(value: int, choices: List[int]) -> List[int]:
    ...
    :return: A one-hot encoding of the value in a list of length
    len(choices) + 1.
...
class MolGraph:
    '''
    A MolGraph represents the graph structure and featurization of a single
    molecule.
    ...
    def __init__(self, smiles: str, args: Namespace):
        ...
        # Get atom features, mapping from atom index to atom features
...
class BatchMolGraph:
    '''
    A BatchMolGraph represents the graph structure and featurization of a batch
    of molecules.
    ...
    combined atom/bond features). #todo: so the bond features are combined atom
    and bond features
    - a_scope: A list of tuples indicating the start and end atom indices for
    each molecule.
    - b_scope: A list of tuples indicating the start and end bond indices for
    each molecule.
    - max_num_bonds: The maximum number of bonds neighboring an atom in this
    batch.
    ...
    def get_components(self) -> Tuple[torch.FloatTensor, torch.FloatTensor,
        ...
        and two lists indicating the scope of the atoms and bonds (i.e. which
    ...
    def get_b2b(self) -> torch.LongTensor:
        '''
        Computes (if necessary) and returns a mapping from each bond index to
        all the incoming bond indices.
    ...
    def get_a2a(self) -> torch.LongTensor:
        '''
        Computes (if necessary) and returns a mapping from each atom index to
        all neighboring atom indices.

        :return: A PyTorch tensor containing the mapping from each bond index
        to all the incodming bond indices.
        '''
        if self.a2a is None:
            # b = a1 --> a2
...
def mol2graph(smiles_batch: List[str],
    ...
    '''
    Converts a list of SMILES strings to a BatchMolGraph containing the batch
    of molecular graphs.

    :param smiles_batch: A list of SMILES strings.
    :param args: Arguments.
    :return: A BatchMolGraph containing the combined molecular graph for the
    molecules
    '''
    mol_graphs = []
